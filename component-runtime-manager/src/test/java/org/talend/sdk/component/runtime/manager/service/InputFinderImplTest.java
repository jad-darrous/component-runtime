...
class InputFinderImplTest {
    ...
    void find() {
        ...
        Assertions.assertEquals(2, InputFinderImplTest.state, "not stopped ?");
    }
    ...
    void serial() throws IOException, ClassNotFoundException {
        final InputFinderImpl implem = new InputFinderImpl("theplugin", null, this::toRecord);

        DynamicContainerFinder.SERVICES.put(InputFinder.class, implem);
    ...
    static class FakeInput implements Input {
        ...
        @Override
        public void start() {
            Assertions.assertEquals(0, InputFinderImplTest.state, "started several times ?");
            InputFinderImplTest.state = 1;
        }

        @Override
        public void stop() {
            Assertions.assertEquals(1, InputFinderImplTest.state, "stopped before started ?");
            InputFinderImplTest.state = 2;
        }

        @Override
        public Object next() {
            Assertions.assertEquals(1, InputFinderImplTest.state, "Was not started or is stopped");
            this.counter++;
            if (this.counter > 3) {
                return null;
            }
